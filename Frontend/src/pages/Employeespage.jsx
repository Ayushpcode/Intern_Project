...
import { Pencil, X, Check, Search, Users } from "lucide-react";
 import { getEmployees } from "../redux/slices/dataAction"; 
 import { updateEmployeeLocal } from "../redux/slices/authSlice"; 

const ALL = "All";
const PER_PAGE = 8;
const STATUSES = ["Active", "Inactive"];
const ROLES = ["Admin", "CEO", "CIO", "Manager", "Sales Executive", "HR"];
...
export default function EmployeesPage() {
  ...
  // Fetch employees from the backend when the page mounts
  useEffect(() => {
    dispatch(getEmployees());
  }, []);

  const filterStatuses = [ALL, ...STATUSES];
  const filterRegions = [ALL, ...REGIONS];

  // data starts as [] (see initialState), so this is always safe to call
  ...
  const startEdit = (emp) => {
    setEditId(emp.id);
    setEditForm({ role: emp.role, status: emp.status });
  };
  const cancelEdit = () => {
    setEditId(null);
    setEditForm({});
  ...
  const saveEdit = (id) => {
    dispatch(updateEmployeeLocal({ id, changes: editForm }));
    setEditId(null);
    setEditForm({});
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-end gap-4">
        <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
          <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider px-1">
            Search
          </label>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50">
                {["Employee", "Role", "DOB", "Email", "Region", "Status", "Action"].map(
                  (h) => (
                    <th
                      key={h}
