...
async function generateEmpId(connection, name) {
  ...
  if (result.rows.length === 0) return `${initials}-1001`;
...
export const approveEmployee = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: "Employee not found or already processed." });
    }
    ...
    await connection.execute(
      `UPDATE employee
       SET emp_id           = :empId,
           ...
           status           = 'ACTIVE'
       WHERE id = :id`,
      { empId, role, region: region.trim(), password: hashedPassword, id },
      { autoCommit: true }
    );
    ...
    return res.status(200).json({
      ...
      data: { emp_id: empId, name: employee.EMP_NAME, role, region },
    });
...
export const rejectEmployee = async (req, res) => {
  ...
  try {
    ...
    const result = await connection.execute(
      `SELECT id FROM employee WHERE id = :id AND status = 'PENDING'`,
      { id }
    );
    ...
    return res.status(200).json({ success: true, message: "Employee registration rejected." });
