...
export const register = async (req, res) => {
  ...
  try {
    ...
    await connection.execute(
      `INSERT INTO employee (emp_name, dob, email, status)
       VALUES (:name, :dob, :email, 'PENDING')`,
      {
        name: name.trim(),
        dob: new Date(dob),
...
export const changePassword = async (req, res) => {
  ...
  try {
    ...
    const result = await connection.execute(
      `SELECT id, emp_id, emp_name FROM employee
       WHERE emp_id = :emp_id AND is_temp_password = 1`,
      { emp_id },
    );
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        success: false,
        message: "Employee not found or password already changed.",
      });
    }
